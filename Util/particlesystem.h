...
struct Particle : Fields...
{
		...
		template <typename ...T>
		constexpr Particle(T &&...x) : Fields{ static_cast<T&&>(x) } ...
		{
			static_assert(sizeof ...(T) == field_count(), "Particle argument count mismatch");
		}
		...
		{
			return { ( field_prop {
					reinterpret_cast<std::uintptr_t>(static_cast<Particle*>(nullptr)->Fields::value()),
							Fields::count(),
							Fields::is_floating_point()
				})...};
		}
};
...
template <typename T>
struct field_detail_t
{
		static_assert(std::is_arithmetic<T >::value, "Particle types must be of integral types");
		typedef T type_t;
...
{	\
	...
	template<typename __T, typename = typename std::enable_if<field_detail::count == 1 && sizeof(__T)>::type>	\
	type_name##_field_t(__T a) : type_name{ a } {}	\
	template<typename __T, typename = typename std::enable_if<field_detail::count == 2 && sizeof(__T)>::type>	\
	type_name##_field_t(__T a, __T b) : type_name{ a, b } {}	\
	template<typename __T, typename = typename std::enable_if<field_detail::count == 3 && sizeof(__T)>::type>	\
	type_name##_field_t(__T a, __T b, __T c) : type_name{ a, b, c } {}	\
...
class ParticleSystem
{
	...
	private:
		void init_programs(size_t field_count,
						   const char **field_names);

		void init_buffers(size_t particle_size,
						  size_t field_count,
						  field_prop *field_deatils,
						  size_t total_particle_count,
						  size_t init_particle_count,
						  void * init_buffer);
	...
	public:
		...
		void setRenderShaders(const Shader &render_vert, const Shader &render_geom, const Shader &render_frag);
		void addTexture(Texture t);
		void setBlendFunc(int key, int func);

		template <typename ...Fields>
		void init(size_t max_particles, std::vector<Particle<Fields... > > starting_set)
		{
